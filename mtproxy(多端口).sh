...
info_mtp() {
    ...
    if [ $? == 1 ]; then
        ...
        for i in $ports
        do
            echo -e "TG一键链接: https://t.me/proxy?server=${public_ip}&port=${i}&secret=${client_secret}"
            echo -e "TG一键链接: tg://proxy?server=${public_ip}&port=${i}&secret=${client_secret}\n"
        done
    else
